...
## Androidアプリ化（TWA）と課金の注意

Google Play は「アプリ内のデジタル機能の購入は Google Play 課金を使う」ルールがあるため、本アプリは **アプリ（TWA）モードでは課金導線（価格・購入ボタン・プランナビ）を出さない** 設計にしています（＝アプリは無料お試し版、課金はWeb版で）。

### 仕組み
- TWA の起動URLに **`?mode=app`** を付けると `app_mode` Cookie が立ち、課金UIが非表示になります。
- 通常のブラウザ（Web版）では従来どおり料金・購入が表示されます。
- `?mode=web` で解除できます（検証用）。

### TWA生成時の設定
- Bubblewrap / PWABuilder で AAB を作る際、**起動URL（Launch URL）を `https://<本番URL>/?mode=app`** に設定してください。
- これにより、アプリから開いた場合のみ課金UIが隠れ、Google Play のルールに準拠します。
- 課金はWeb版（ブラウザ）で行う運用です（アプリ内からは決済に誘導しない）。
